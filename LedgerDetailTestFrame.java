...
//        getLedgerDetail();
    ...
    private void getBatch(){
     ...
     private void addRecord(){
            ...
            try {
            int row = DatabaseManager.addLedgerDetail(seatListId,schemeId,schemePart,semester,courseNo,rollNo,obtMarks,qp,grade,result,remarks);
            if(row>0){
     ...
     private void deleteRecord(){    
        ...
        if(bean==null)return;
        try {
            int row = DatabaseManager.deleteLedgerDetail(bean.getSchemeId());
            if(row>0){
...
//            }
       ...
       schemePartComboBox1.setSelectedIndex(bea.getSchemePart()-1);
        semesterComboBox2.setSelectedIndex(bea.getSemester()-1);
     if(bea==null )return; 
